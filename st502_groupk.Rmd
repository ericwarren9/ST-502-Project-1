...
```{r}
Raw_CI <- function(df, B = 200, alpha = 0.05) {
  raw <- apply(df, MARGIN = 1, FUN = function(combo) {
    set.seed(999)
    n <- combo['n']
    p <- combo['p']
    y_i <- combo['y_i']
...
```{r}
Bootstrap_t_CI <- function(df, B = 200, alpha = 0.05) {
  boot_t <- apply(df, MARGIN = 1, FUN = function(combo) {
    set.seed(999)
    n <- combo['n']
    p <- combo['p']
    y_i <- combo['y_i']
